...
export class PeopleService {
    ...
    async getPeople() {
        const persons = await this.personModel.find({published: true}).sort({index: 1}).exec();
        return persons
            .map(person => ({
                id: person.id,
                name: person.name,
    ...
    async getPeopleWithArrangement(arrangement) {
        const persons = await this.personModel.find({arrangement, published: true}).sort({index: 1}).exec();
        return persons
            .map(person => ({
                id: person.id,
                name: person.name,
